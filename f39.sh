...
[[ "$(cat $mnt/etc/fstab | grep /home/$user/.config)" ]] && echo "tmpfs    /home/$user/.cache    tmpfs   rw,nodev,nosuid,uid=$user,size=2G   0 0" > $mnt/etc/fstab
cat $mnt/etc/fstab
...
echo 'export PS1="(chroot) $PS1"     # So we know when we are in chroot. I don't think this works
...
chown user:user /home/$user/.local/bin/setup-iwd.sh
...
mkdir -p ~/.config ~/.local/share/{bin,applications}
...
[General]
count=1
rules=1' > $mnt/home/$user/.config/kwinrulesrc
chown user:user $mnt/home/$user/.config/kwinrulesrc
...
chown user:user $mnt/home/$user/.local/bin/snapper.sh
